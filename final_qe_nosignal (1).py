...
problem -- ZERO-SIGNAL FILTER VARIANT, windowed to l=10-5000. No ViT is
loaded or run anywhere in this script.

This is a variant of the l=10-5000 windowed QE script that changes exactly
one thing about the QE's assumptions, and nothing about the data or the
l-window:

  * Cl_TT(eps), the power spectrum the QE *assumes* for the T map when
    building its inverse-variance filter, no longer includes the signal
    term. Normally Cl_TT(eps) = Cl_T_signal + eps^2 * Cl_noise. Here it is
    Cl_TT(eps) = eps^2 * Cl_noise ONLY -- i.e. the filter is told "this map
    has no signal power, only noise," even though the map handed to the QE
    (Tnoise_ring = T_ring + eps*noise_ring) still contains the real signal.
    This tests how the l=10-5000 windowed QE degrades when its noise model
    is misspecified in this specific way.

  * Only eps=1.0 (full noise) is evaluated. eps=0.0 is not run here: at
    eps=0.0, zeroing the signal in Cl_TT would leave Cl_TT identically
    zero, which is singular for 1/Cl_TT -- this is a hard constraint of
    the zero-signal filter itself, independent of the l-window, so eps=0.0
    is excluded rather than crashing or being silently patched around.

Everything else is UNCHANGED from the l=10-5000 script:
  - Test data: the synthetic held-out realization (test_realization_*
    _seed64.npy), regenerated via camb/hp.synfast if missing, using the
    identical procedure to the main pipeline -- NOT the FLAMINGO real maps.
  ...
  - Loading of TEST_FILES stays serial (np.load per file/bin).
  - v_hat is saved bundled into RESULTS_FILE alongside cl_true, cl_qe,
    cl_cross, and Nl (not as separate per-bin .npy files).
  - ThreadPoolExecutor(max_workers=3) in run_qe_binned, matching the
    submission script's --cpus-per-task 12 / OMP_NUM_THREADS=4 speedup
    (3 concurrent bin-threads x 4 OpenMP threads each = 12).

Output filenames/location: results and summary plot both live under
final_qe_nosignal/ (new directory, distinct from final_qe_generated/ and
final_qe_flamingo/), with names encoding the filter range and the
"nosignal" tag, so this run does not overwrite any previous run's outputs.
The ensemble spectra cache is left with its original shared name/location
since it depends only on the training realizations, which are unchanged
and unaffected by this variant.
...
Design notes (see conversation for full reasoning):
  ...
  2. Cl_TT(eps) = eps^2 * Cl_noise ONLY (zero-signal variant -- see above).
     Cl_T_signal_avg is still computed as part of the shared ensemble-
     spectra cache but is intentionally unused here.
  3. The QE's filter spans l=10 to l=5000 inclusive. MSE is reported over
     that same range, l=[10, 5000].
  4. Evaluated ONLY at eps=1.0 (full noise) -- eps=0.0 excluded due to the
     1/Cl_TT singularity described above.
...
OUT_DIR = os.path.join(DATA_DIR, "final_qe_nosignal")
...
# --- TEST DATA: synthetic held-out realization -- UNCHANGED from the
# l=10-5000 script. NOT the FLAMINGO real maps.
TEST_FILES = {
...
# Only the requested scenario: eps=1.0 (full noise), zero-signal filter.
# eps=0.0 excluded -- see module docstring (1/Cl_TT singularity).
QE_EPS_LIST = [1.0]
...
# filter, the eps list, or the zero-signal variant, only on the (unchanged)
# training realizations.
ENSEMBLE_SPECTRA_CACHE = os.path.join(DATA_DIR, "qe_binned_ensemble_spectra.npz")

# Routed into final_qe_nosignal/ with filter range + "nosignal" tag in the
# name so this never overwrites any previous run's outputs.
RESULTS_FILE = os.path.join(OUT_DIR, f"qe_binned_results_l{QE_LMIN_FILTER}-{QE_LMAX_FILTER-1}_nosignal.npz")
SUMMARY_PLOT = os.path.join(OUT_DIR, f"qe_binned_summary_l{QE_LMIN_FILTER}-{QE_LMAX_FILTER-1}_nosignal.png")
...
def generate_test_realization_if_missing():
    ...
    # bit-identical to what the ViT was actually scored against.
    np.random.seed(TEST_SEED)
...
def cl_tt_for_eps(eps):
    """ZERO-SIGNAL VARIANT: Cl_TT(eps) = eps^2 * Cl_noise ONLY.

    The signal term Cl_T_signal_avg is deliberately dropped -- the QE's
    filter is built as if the T map carried no signal power, only noise.
    Cl_T_signal_avg is still computed above (it's part of the shared
    ensemble-spectra cache) but is intentionally unused here.

    Note this is purely about what Cl_TT the *filter* assumes; the actual
    map fed to the QE (Tnoise_ring below) is unchanged and still contains
    the real signal plus eps*noise. Only called with eps=1.0 in this
    script -- at eps=0.0 this would be identically zero, singular for
    1/Cl_TT (see module docstring).
    """
    return (eps ** 2) * Cl_noise_avg
...
def compute_Nl(Cl_tau_b, Cl_TT, lmax=LMAX, lmin_filter=QE_LMIN_FILTER, lmax_filter=QE_LMAX_FILTER):
    ...
    shared (zero-signal) Cl_TT(eps). lmin_filter/lmax_filter bound the
    wigner-3j sum to the same l-range the estimator itself is restricted
    to, so Nl is consistent with a windowed (not full-range) QE."""
    inv_Cl_TT = 1.0 / Cl_TT
...
def run_qe_binned(Tnoise_ring, tau_rings: dict, Cl_tau_avg: dict, Cl_TT: np.ndarray, lmax=LMAX):
    ...
    4-channel input [Tnoise, tau0, tau1, tau2]. Cl_TT here is the
    zero-signal variant from cl_tt_for_eps.
...
##  ViT is evaluated against) and run the QE at eps=1.0 with the
##  zero-signal filter, l=10-5000 window
##=====================================
...
# cl_true doesn't depend on eps -- compute once per bin
...
for eps in QE_EPS_LIST:
    print(f"\n=== QE reconstruction at eps={eps} (zero-signal filter, l=10-5000) ===")
    Tnoise_ring = (eps * noise_ring).astype(np.float32)
...
##  file) + summary plot -- written to final_qe_nosignal/, never
...
# One Cl comparison plot per bin: true vs QE(eps=1, zero-signal filter), windowed l range
...
for i, b in enumerate(BIN_NAMES):
    ...
    ax.plot(ell[valid_mask], qe_results[b][1.0]["cl_true"][valid_mask], color='black', linewidth=2, label="true")
    ax.plot(ell[valid_mask], qe_results[b][1.0]["cl_qe"][valid_mask], label="QE (eps=1, zero-signal filter)")
...
print("\nDone. This script produced ZERO-SIGNAL-FILTER QE-only results (l=10..5000 window) using:")
