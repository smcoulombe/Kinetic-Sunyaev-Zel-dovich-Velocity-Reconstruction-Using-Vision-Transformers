"""
SLURM batch job: heavy-compute half of the ViT analysis -- FLAMINGO DATA
(same pipeline the QE flamingo script and vit_compute_job_base_flamingo.py
used), FINE-TUNED CHECKPOINT (best val_loss), comparison-format outputs.

This is a modified copy of vit_compute_job_base_flamingo.py. The ONLY
change is which model weights are loaded:

  * Instead of the original curriculum run's stage-24-end checkpoint
    (model_weights_{SNAPSHOT_TAG}_stage24_end.pt), this script loads the
    BEST checkpoint from the flamingo-rotation fine-tuning run
    (finetune_from_stage24.py, submitted with
    SNAPSHOT_TAG=binned_finetune_flamingo_rot_v1 per that run's .sh):

        model_weights_binned_finetune_flamingo_rot_v1_best.pt

    This is BEST_CHECKPOINT_FILE from that script -- i.e. the weights from
    the epoch with the lowest validation loss during fine-tuning, NOT
    necessarily the literal last epoch trained (early stopping via
    patience means these can differ). If you want the literal final-epoch
    weights instead, use model_weights_binned_finetune_flamingo_rot_v1.pt
    (no "_best" suffix) -- CHECKPOINT_FILE below would need to change to
    point there instead.

  * There is no STAGE_TO_LOAD / SOURCE_STAGE_LABEL concept for this
    checkpoint -- it isn't a numbered curriculum stage, it's the
    fine-tuning run's own best snapshot, identified purely by its
    SNAPSHOT_TAG (FINETUNE_SNAPSHOT_TAG below, distinct from the original
    run's SNAPSHOT_TAG used elsewhere in this file family).

Test data is UNCHANGED from vit_compute_job_base_flamingo.py: the same
flamingo_real_{name}_nside2048.npy files, loaded the same simple way (plain
np.load, no ud_grade/floor-clip/normalization), matching
qe_binned_standalone_l10_5000_flamingo.py's data source exactly. Note this
is the SAME held-out original (un-rotated) FLAMINGO map that
finetune_from_stage24.py itself used as its TEST_FILES / test_snapshot()
target -- so this evaluation reuses the fine-tune run's own held-out test
set, which is appropriate here since that data was never part of the
fine-tune's train/val pool (only the 4 ROTATED maps were).

Everything else -- model architecture, loss functions, patch/geometry
helpers, VIT_EPS_LIST=[0.0, 1.0], no l-filtering (VIT_LMIN_FILTER=1, no
upper cutoff), serial loading/reconstruction, and the bundled save format
(cl_true, cl_vit, cl_cross, v_hat all in ONE RESULTS_FILE) -- is IDENTICAL
to vit_compute_job_base_flamingo.py.

Output: RESULTS_FILE lives under final_vit_finetuned_flamingo/ (new
directory), with a name tagged "_finetuned", so this run's outputs never
collide with the stage-24-checkpoint flamingo run's outputs
(final_vit_base_flamingo/).

Run via: sbatch submit_vit_finetuned_flamingo.sh
(which just calls: python vit_compute_job_finetuned_flamingo.py)

ASSUMPTIONS -- check before trusting:
  - FINETUNE_SNAPSHOT_TAG = "binned_finetune_flamingo_rot_v1", confirmed
    from that run's submission script (export SNAPSHOT_TAG=...).
  - Model architecture matches finetune_from_stage24.py's SimpleViT exactly:
    in_ch=4, out_ch=3, embed_dim=128, depth=5, n_heads=4, mlp_ratio=4,
    patch_size=8, eps_max=EPS_MAX=1.5 (max of that script's EPS_LIST) --
    same architecture as the original curriculum run, since fine-tuning
    only continued training from those weights, never changed the model
    shape.
"""
...
# ==== constants -- must match the fine-tuning script ====
# NOTE: distinct from SNAPSHOT_TAG used in the stage-24 comparison scripts --
# this identifies the FINE-TUNING run, not the original curriculum run.
FINETUNE_SNAPSHOT_TAG = "binned_finetune_flamingo_rot_v1"
...
EPS_MAX = 1.5   # max(EPS_LIST) in the training/fine-tuning script -- must match exactly
...
OUT_DIR = os.path.join(DATA_DIR, "final_vit_fine_flamingo")

# CHANGED: fine-tuning run's BEST checkpoint (lowest val_loss epoch), not a
# stage-numbered checkpoint from the original curriculum run.
CHECKPOINT_FILE = os.path.join(DATA_DIR, f"model_weights_{FINETUNE_SNAPSHOT_TAG}_best.pt")
LOSS_SCALE_FILE = os.path.join(DATA_DIR, "binned_loss_scale.npz")
RESULTS_FILE = os.path.join(OUT_DIR, f"vit_results_finetuned_flamingo_{FINETUNE_SNAPSHOT_TAG}.npz")

# --- TEST DATA: same flamingo_real_* files, loaded the same simple way,
# as qe_binned_standalone_l10_5000_flamingo.py and
# vit_compute_job_base_flamingo.py -- READ ONLY, no regeneration.
TEST_FILES = {
...
assert os.path.exists(CHECKPOINT_FILE), (
    f"{CHECKPOINT_FILE} not found -- check FINETUNE_SNAPSHOT_TAG (currently "
    f"'{FINETUNE_SNAPSHOT_TAG}') against what the fine-tuning run's .sh actually used."
)
assert os.path.exists(LOSS_SCALE_FILE), f"{LOSS_SCALE_FILE} not found."
for name, p in TEST_FILES.items():
    assert os.path.exists(p), (
        f"{p} not found -- these are the original FLAMINGO fields and cannot be "
        f"generated by this script; make sure they exist on disk before running."
    )
...
##=====================================
##  MODEL  (identical architecture to training/fine-tuning script)
##=====================================
...
model.eval()
print(f"Loaded fine-tuned BEST checkpoint from {CHECKPOINT_FILE}")

##=====================================
##  LOAD FLAMINGO TEST DATA (same files, same loading style, as
##  qe_binned_standalone_l10_5000_flamingo.py and
##  vit_compute_job_base_flamingo.py -- plain np.load, no
##  ud_grade / floor-clip / normalization)
##=====================================
...
print(f"Loaded FLAMINGO test maps from {DATA_DIR} (original, unrotated) -- "
      f"same source as the QE flamingo run and the stage-24-checkpoint ViT flamingo run.")
...
results = {
    ...
    "FINETUNE_SNAPSHOT_TAG": FINETUNE_SNAPSHOT_TAG,
