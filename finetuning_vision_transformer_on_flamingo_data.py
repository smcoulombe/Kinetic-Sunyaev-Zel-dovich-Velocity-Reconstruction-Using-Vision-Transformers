...
Original file is located at
    https://colab.research.google.com/drive/1IqVgNiClVUYig0S3K3SxuL4fgXoBHqrQ
"""
...
def _rot_tag(rot):
    # (45, 135, 0) -> "rot45_135_0", matching the filenames the rotate-and-save script writes.
    a, b, c = rot
...
##  moving_average
##=====================================
# Simple edge-padded box-car smoother, used elsewhere for smoothing loss curves.
...
def reassemble_from_stack(patch_stack, nside_map, nside_patch):
    # Scatter a stack of predicted patches back into one full-sky NEST map,
    # using (and lazily caching) the pixel-index mapping from
    # precompute_patch_pixel_indices.
...
def reconstruct_full_maps_multi(model, Tnoise_map_nest, tau_maps_nest, nside_map, nside_patch, eps, batch_size=256):
    # Run the model over every patch of a full-sky map (batched for memory),
    # at a single fixed eps, and reassemble the per-channel predictions back
    # into full-sky maps.
...
def find_source_cache_n(target_n):
    # Look for a smaller, already-complete patch cache (fewer realizations)
    # under the same tag, so build_and_save_stage_chunked_binned can copy
    # that work forward instead of reprocessing those realizations.
...
def build_and_save_stage_chunked_binned():
    # Build the patch cache for all N_REALIZATIONS rotated maps, writing to
...
def radial_power_spectrum(patch_batch, n_bins=N_SPEC_BINS):
    # 2D FFT power spectrum of a batch of square patches, averaged into
    # n_bins log-spaced radial frequency bins (an isotropic power spectrum
    # per patch).
...
def spectral_loss_fn(pred, target, n_bins=N_SPEC_BINS, eps=SPEC_LOSS_EPS):
    # MSE between log radial power spectra of prediction vs. target,
    # flattening the channel dimension into the batch so each output
    # channel's spectrum is scored independently.
    B, C, H, W = pred.shape
...
def compute_stage_weights(active, tier1_set, tier2_set, prev_weights, eps0_weight,
    ...
    # get trimmed toward retire_floor first, then tier2, then any leftover
    # deficit is spread evenly across the "durable" (non-retiring) values.
    # A surplus (deficit < 0) is instead added evenly to whichever pool is
    # available. This is only used here to reconstruct what the ORIGINAL
    # curriculum run's weights looked like at stage SOURCE_STAGE_LABEL --
    # see the "FIXED EPS MIXTURE" section below.
...
def build_curriculum_stages(eps_list, samples_per_eps, eps0_start=1.0, eps0_end=0.3,
                             base_weight=0.12, retire_floor=0.02):
    # Rebuild the full stage-by-stage weight schedule of the ORIGINAL
    # curriculum run (one new eps value activated per stage, eps=0's share
    # linearly ramped down from eps0_start to eps0_end). This fine-tune
    # script never trains through these stages -- it only needs stage
    # SOURCE_STAGE_LABEL's weight dict as its fixed mixture (see below).
...
def save_resume_state(next_epoch, epochs_since_improvement, best_val_loss,
    ...
    # Persist just enough state (epoch counter, early-stopping counters,
    # best-loss bookkeeping) to resume training exactly where it left off
    # after a restart. global_best_epoch is stored as -1 (not None) since
    # npz can't hold None.
...
# --- checkpoint init/resume logic: ---
#  - normal case: if this fine-tune's own CHECKPOINT_FILE already exists,
#    resume from it (below).
#  - first run (or FORCE_RESTART=1): initialize instead from the ORIGINAL
#    curriculum run's stage-24 checkpoint, SOURCE_CHECKPOINT_FILE.
if FORCE_RESTART:
...
# --- main training loop ---
# Each epoch: resample the fixed-mixture train set, run one train pass and
# one val pass (tracking total + per-component losses), checkpoint, and
# occasionally take a full test-set snapshot. Stops early once val loss
# hasn't improved by more than MIN_DELTA (relative) for PATIENCE epochs.
